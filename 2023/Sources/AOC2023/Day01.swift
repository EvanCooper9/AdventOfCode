...
struct Day01: Solution {
        ...
        result { line, reversed in
            ...
            for character in line {
                if let number = character.wholeNumberValue {
                    ...
                    if let match = Constants.speltDigits.first(where: { key, _ in stringBuffer.contains(key) }) {
                        return match.value
                    }
                }
            }
    ...
    private func result(lineHandler: (String, Bool) -> Int?) -> Int {
        input.reduce(into: 0) { partialSum, line in
            ...
            if let first, let second, let number = Int("\(first)\(second)") {
                partialSum += number
            }
        }
    }
